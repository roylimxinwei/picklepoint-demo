...
const COLORS = {
  spectator: { primary: "#94A3B8", accent: "#CBD5E1", bg: "rgba(148,163,184,0.08)", border: "rgba(148,163,184,0.25)", gradient: "linear-gradient(135deg, #64748B, #94A3B8)" },
...
function TopBar({ onBack, title, right, borderColor = "rgba(255,255,255,0.06)" }) {
  return (
    <div style={{ padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${borderColor}`, flexShrink: 0 }}>
      <button onClick={onBack} style={{ background: "none", border: "none", color: "#64748B", cursor: "pointer", fontSize: 14, fontFamily: "DM Sans" }}>← Back</button>
      <div style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 17, color: "#E8ECF4" }}>{title}</div>
...
function Badge({ text, color, bg }) { return <span style={{ padding: "3px 10px", borderRadius: 6, fontFamily: "JetBrains Mono", fontSize: 10, fontWeight: 600, background: bg, color, whiteSpace: "nowrap" }}>{text}</span>; }

function Chip({ label, active, onClick, color = "#3B82F6" }) {
  return (
    <button onClick={onClick} style={{
      padding: "6px 14px", borderRadius: 20, fontSize: 12, fontFamily: "DM Sans", fontWeight: 600, cursor: "pointer",
      background: active ? color + "22" : "rgba(255,255,255,0.03)",
      border: `1px solid ${active ? color + "55" : "rgba(255,255,255,0.08)"}`,
      color: active ? color : "#64748B", transition: "all 0.15s", whiteSpace: "nowrap"
    }}>{label}</button>
  );
}
...
export default function PicklePoint() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", minHeight: "100vh", background: "#0A0F1A", color: "#E8ECF4" }}>
      ...
      {view === "organizer" && authed && !selectedComp && <OrgCompList onSelect={setSelectedComp} onBack={() => goTo("landing")} />}
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, background: "linear-gradient(170deg, #0A0F1A 0%, #111827 50%, #0A0F1A 100%)", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, textAlign: "center", maxWidth: 440, width: "100%" }}>
        <div style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 3, marginBottom: 8 }}>INTERACTIVE PROTOTYPE</div>
        <h1 className="pp-landing-title" style={{ fontFamily: "Outfit", fontSize: 48, fontWeight: 800, margin: "0 0 4px", background: "linear-gradient(135deg, #FFF 30%, #64748B 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>PicklePoint</h1>
        <div style={{ fontSize: 13, color: "#475569", fontFamily: "JetBrains Mono", marginBottom: 40 }}>Real-time tournament management</div>
        <div style={{ textAlign: "left", fontSize: 11, color: "#334155", fontFamily: "JetBrains Mono", letterSpacing: 2, marginBottom: 12, paddingLeft: 4 }}>SELECT YOUR ROLE</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {roles.map((r, i) => (
            ...
            >
              <div style={{ fontSize: 24, width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", background: r.colors.bg, borderRadius: 10, border: `1px solid ${r.colors.border}`, flexShrink: 0 }}>{r.icon}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 16, color: r.colors.primary }}>{r.label}</div>
                <div style={{ fontSize: 12, color: "#64748B", marginTop: 1 }}>{r.desc}</div>
              </div>
              <div style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "#475569", background: "rgba(255,255,255,0.04)", padding: "4px 8px", borderRadius: 6, flexShrink: 0 }}>{r.auth}</div>
            </button>
          ))}
        </div>
        <div style={{ marginTop: 28, fontSize: 11, color: "#1E293B", fontFamily: "JetBrains Mono" }}>↑ increasing permissions ↓</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "#0A0F1A" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 360, width: "100%", animation: "slideUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}><div style={{ fontSize: 40, marginBottom: 12 }}>👀</div><div style={{ fontFamily: "Outfit", fontSize: 22, fontWeight: 700 }}>Watch Live Scores</div><div style={{ fontSize: 13, color: "#64748B", marginTop: 4 }}>Enter the tournament code to view</div></div>
          <label style={{ display: "block", fontSize: 12, fontFamily: "JetBrains Mono", color: "#64748B", marginBottom: 6 }}>TOURNAMENT CODE</label>
          <input value={code} onChange={e => { setCode(e.target.value.replace(/\D/g, "").slice(0, 5)); setError(""); }} placeholder="00000" style={{ width: "100%", padding: "14px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(148,163,184,0.2)", borderRadius: 10, color: "#E8ECF4", fontSize: 24, fontFamily: "JetBrains Mono", textAlign: "center", letterSpacing: 8, outline: "none", marginBottom: 16 }} />
          <label style={{ display: "block", fontSize: 12, fontFamily: "JetBrains Mono", color: "#64748B", marginBottom: 6 }}>VERIFY YOU'RE HUMAN</label>
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(148,163,184,0.15)", borderRadius: 10, padding: 14, marginBottom: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 8, color: "#94A3B8" }}>What is 4 + 3?</div>
            <input value={captcha} onChange={e => { setCaptcha(e.target.value); setError(""); }} placeholder="Answer" style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(148,163,184,0.15)", borderRadius: 8, color: "#E8ECF4", fontSize: 16, outline: "none" }} />
          </div>
          {error && <div style={{ color: "#EF4444", fontSize: 13, marginBottom: 12, textAlign: "center" }}>{error}</div>}
          <button onClick={submit} style={{ width: "100%", padding: 14, background: COLORS.spectator.gradient, border: "none", borderRadius: 10, color: "#0A0F1A", fontFamily: "Outfit", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>View Live Scores →</button>
        </div>
      </div>
    </div>
  );
...
function AuthScreen({ role, onAuth, onBack }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "#0A0F1A" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 360, width: "100%", animation: "slideUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}><div style={{ fontSize: 40, marginBottom: 12 }}>{icons[role]}</div><div style={{ fontFamily: "Outfit", fontSize: 22, fontWeight: 700, color: cc.primary }}>{labels[role]} {mode === "login" ? "Login" : "Sign Up"}</div></div>
          <button onClick={onAuth} style={{ width: "100%", padding: 13, background: "#FFF", border: "none", borderRadius: 10, color: "#1F2937", fontFamily: "DM Sans", fontWeight: 600, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}><div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.06)" }} /><span style={{ fontSize: 12, color: "#475569" }}>or</span><div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.06)" }} /></div>
          <label style={{ display: "block", fontSize: 12, fontFamily: "JetBrains Mono", color: "#64748B", marginBottom: 6 }}>EMAIL</label>
          <input placeholder="you@email.com" type="email" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: `1px solid ${cc.border}`, borderRadius: 10, color: "#E8ECF4", fontSize: 15, outline: "none", marginBottom: 12 }} />
          <label style={{ display: "block", fontSize: 12, fontFamily: "JetBrains Mono", color: "#64748B", marginBottom: 6 }}>PASSWORD</label>
          <input placeholder="••••••••" type="password" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: `1px solid ${cc.border}`, borderRadius: 10, color: "#E8ECF4", fontSize: 15, outline: "none", marginBottom: 20 }} />
          <button onClick={onAuth} style={{ width: "100%", padding: 13, background: cc.gradient, border: "none", borderRadius: 10, color: "#0A0F1A", fontFamily: "Outfit", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>{mode === "login" ? "Log In" : "Create Account"} →</button>
          <div style={{ textAlign: "center", marginTop: 16 }}><button onClick={() => setMode(mode === "login" ? "signup" : "login")} style={{ background: "none", border: "none", color: cc.primary, cursor: "pointer", fontSize: 13 }}>{mode === "login" ? "Don't have an account? Sign up" : "Already have an account? Log in"}</button></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        {["mine", "find"].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            color: tab === t ? "#E8ECF4" : "#475569", fontFamily: "DM Sans", fontWeight: 600, fontSize: 13, cursor: "pointer"
      ...
      <div style={{ padding: 14 }}>
        {tab === "mine" ? (
          <>
            <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 2, marginBottom: 10 }}>REGISTERED</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 16 }}>{comp.name}</span>
                      <Badge text="LIVE" color="#22C55E" bg="rgba(34,197,94,0.15)" />
                    </div>
                    <div style={{ fontSize: 13, color: "#64748B" }}>{comp.date} · {comp.venue}</div>
          ...
          <>
            ...
            <div style={{ position: "relative", marginBottom: 12 }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search competitions, venues, organizers..." style={{
                width: "100%", padding: "11px 14px 11px 38px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(239,68,68,0.15)",
                borderRadius: 10, color: "#E8ECF4", fontSize: 14, outline: "none"
              }} />
            </div>
            {/* Filters */}
            ...
            <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 2, marginBottom: 10 }}>{filtered.length} COMPETITIONS FOUND</div>
            {filtered.map((comp, i) => (
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                      <span style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 15, color: "#E8ECF4" }}>{comp.name}</span>
                    ...
                    <div style={{ fontSize: 12, color: "#64748B" }}>{comp.date} · {comp.venue}</div>
                    <div style={{ fontSize: 12, color: "#475569", marginTop: 2 }}>by {comp.organizer}</div>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
                    <div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 16, color: COLORS.player.primary }}>{comp.fee}</div>
                    <div style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "#475569", marginTop: 2 }}>{comp.playerCount}/{comp.maxPlayers}</div>
                    {comp.status === "registration" && (
                      <button style={{ marginTop: 8, padding: "6px 14px", background: COLORS.player.gradient, border: "none", borderRadius: 8, color: "white", fontSize: 12, fontFamily: "Outfit", fontWeight: 600, cursor: "pointer" }}>Register</button>
                    )}
                  </div>
                </div>
              </div>
    ...
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: 24, paddingTop: 48 }}>
        <div style={{ maxWidth: 360, width: "100%", textAlign: "center", animation: "slideUp 0.4s ease" }}>
          <div style={{ fontFamily: "Outfit", fontSize: 20, fontWeight: 700, marginBottom: 4 }}>{comp.name}</div>
          <div style={{ fontSize: 13, color: "#64748B", marginBottom: 28 }}>{comp.date} · {comp.venue}</div>
          {!waiting ? (
            <>
              <div style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#64748B", letterSpacing: 2, marginBottom: 16 }}>SHOW THIS TO THE ORGANIZER</div>
              {/* Simulated QR Code */}
              <div style={{
              ...
              <div style={{ fontFamily: "JetBrains Mono", fontSize: 18, letterSpacing: 4, color: "#E8ECF4", marginBottom: 4 }}>PB-83472-ASM</div>
              <div style={{ fontSize: 12, color: "#475569", marginBottom: 24 }}>Your unique check-in code</div>
              ...
              <button onClick={() => setWaiting(true)} style={{
                width: "100%", padding: 14, background: COLORS.player.gradient, border: "none",
                borderRadius: 10, color: "white", fontFamily: "Outfit", fontWeight: 700, fontSize: 16, cursor: "pointer"
            ...
            <>
              ...
              <div style={{ fontFamily: "JetBrains Mono", fontSize: 13, color: COLORS.player.primary, marginBottom: 8, animation: "pulse 2s infinite" }}>Waiting for organizer to scan...</div>
              <div style={{ fontSize: 12, color: "#475569", marginBottom: 24 }}>Show your screen to the check-in desk</div>
...
function SpectatorLive({ matches, focusMatch, setFocusMatch, onBack }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      <TopBar onBack={onBack} title="Live Scores" right={<div style={{ display: "flex", alignItems: "center", gap: 5 }}><LiveDot /><span style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#22C55E" }}>LIVE</span></div>} />
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}><Badge text={`Court ${m.court}`} color={done ? "#64748B" : "#22C55E"} bg={done ? "rgba(255,255,255,0.05)" : "rgba(34,197,94,0.15)"} /><span style={{ fontSize: 10, color: "#475569", fontFamily: "JetBrains Mono" }}>{m.round}</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>{isRef && !done && <LiveDot />}<span style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: done ? "#475569" : isRef ? "#22C55E" : "#64748B" }}>{done ? "FINAL" : isRef ? "LIVE" : "IN PROGRESS"}</span></div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ flex: 1 }}><div style={{ fontSize: 12, fontWeight: 500, color: "#94A3B8", marginBottom: 2 }}>{teamDisplay(m.teamA, true)}</div><div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 32, color: done ? (m.matchScore.a > m.matchScore.b ? "#22C55E" : "#64748B") : "#60A5FA" }}>{done || !isRef ? m.matchScore.a : m.currentGame.a}</div></div>
                <div style={{ fontFamily: "JetBrains Mono", fontSize: 13, color: "#1E293B", margin: "0 8px", marginTop: 16 }}>vs</div>
                <div style={{ flex: 1, textAlign: "right" }}><div style={{ fontSize: 12, fontWeight: 500, color: "#94A3B8", marginBottom: 2 }}>{teamDisplay(m.teamB, true)}</div><div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 32, color: done ? (m.matchScore.b > m.matchScore.a ? "#22C55E" : "#64748B") : "#F87171" }}>{done || !isRef ? m.matchScore.b : m.currentGame.b}</div></div>
              </div>
              {!done && <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.04)", fontSize: 10, fontFamily: "JetBrains Mono", color: "#334155" }}>{isRef ? "🏁 Referee · point-by-point" : "🏓 Player scoring · per game"}{!isRef && ` · Match: ${m.matchScore.a}-${m.matchScore.b}`}</div>}
              {m.games.length > 0 && <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", gap: 6, justifyContent: "center" }}>{m.games.map((g, j) => <span key={j} style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#475569", background: "rgba(255,255,255,0.03)", padding: "2px 8px", borderRadius: 4 }}>G{j + 1}: {g.a}-{g.b}</span>)}</div>}
            </button>
          );
        })}
      </div>
...
function MatchFocus({ match: m, onBack }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A", display: "flex", flexDirection: "column" }}>
      <TopBar onBack={onBack} title={`Court ${m.court}`} right={<div style={{ display: "flex", alignItems: "center", gap: 5 }}>{!done && <LiveDot color={isRef ? "#22C55E" : "#64748B"} />}<span style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: done ? "#64748B" : "#22C55E" }}>{done ? "FINAL" : "LIVE"}</span></div>} />
      <div style={{ textAlign: "center", padding: "10px 0 0" }}>
        <div style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: "#475569" }}>{m.division} · {m.round}</div>
        <div style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: "#64748B", marginTop: 2 }}>{isRef ? "🏁 Referee · point-by-point" : "🏓 Player scoring · game-by-game"}</div>
        {!done && <div style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: "#475569", marginTop: 2 }}>Game {m.gameNum}/3 | Match: {m.matchScore.a}-{m.matchScore.b}</div>}
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6 }}>
        <div style={{ fontSize: 15, fontWeight: 500, color: "#94A3B8" }}>{teamDisplay(m.teamA)}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ fontFamily: "JetBrains Mono", fontSize: 18, color: "#1E293B" }}>:</div>
        ...
        <div style={{ fontSize: 15, fontWeight: 500, color: "#94A3B8" }}>{teamDisplay(m.teamB)}</div>
        {!isRef && !done && <div style={{ marginTop: 8, padding: "8px 16px", background: "rgba(255,255,255,0.03)", borderRadius: 8, fontFamily: "JetBrains Mono", fontSize: 11, color: "#475569" }}>Updates when each game ends</div>}
      </div>
      {m.games.length > 0 && <div style={{ padding: "16px 24px", borderTop: "1px solid rgba(255,255,255,0.06)" }}><div style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#475569", marginBottom: 8, textAlign: "center" }}>GAME HISTORY</div><div style={{ display: "flex", gap: 8, justifyContent: "center" }}>{m.games.map((g, i) => <div key={i} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "10px 18px", textAlign: "center" }}><div style={{ fontFamily: "JetBrains Mono", fontSize: 9, color: "#475569", marginBottom: 4 }}>GAME {i + 1}</div><div style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 18 }}><span style={{ color: g.a > g.b ? "#22C55E" : "#64748B" }}>{g.a}</span><span style={{ color: "#1E293B" }}> - </span><span style={{ color: g.b > g.a ? "#22C55E" : "#64748B" }}>{g.b}</span></div></div>)}</div></div>}
    </div>
  );
}
...
function PlayerLive({ matches, onViewMatch, onBack }) {
  const myMatch = matches.find(m => m.id === 3); const active = matches.filter(m => m.status === "active");
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      <TopBar onBack={onBack} title="My Tournament" right={<span style={{ fontSize: 18, cursor: "pointer" }}>🔔</span>} />
      <div style={{ padding: 14 }}>
        <div style={{ background: "linear-gradient(135deg, rgba(239,68,68,0.1), rgba(239,68,68,0.03))", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 16, padding: 18, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><span style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#22C55E", letterSpacing: 2 }}>ON COURT NOW</span><div style={{ display: "flex", alignItems: "center", gap: 4 }}><LiveDot /><span style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#22C55E" }}>LIVE</span></div></div>
          <div style={{ fontSize: 12, color: "#64748B", marginBottom: 8 }}>Court 3 · {myMatch?.division} · {myMatch?.round}</div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ fontSize: 12, color: "#94A3B8" }}>{teamDisplay(myMatch?.teamA, true)}</div><div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 36, color: "#60A5FA" }}>{myMatch?.currentGame.a}</div></div>
            <div style={{ fontFamily: "JetBrains Mono", color: "#1E293B", marginTop: 14 }}>vs</div>
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, color: "#94A3B8" }}>{teamDisplay(myMatch?.teamB, true)}</div><div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 36, color: "#F87171" }}>{myMatch?.currentGame.b}</div></div>
          </div>
          <div style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#475569", textAlign: "center", marginTop: 6 }}>Game {myMatch?.gameNum} · Match: {myMatch?.matchScore.a}-{myMatch?.matchScore.b}</div>
        </div>
        <div style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.15)", borderRadius: 14, padding: 16, marginBottom: 12 }}>
          <div style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#F59E0B", letterSpacing: 2, marginBottom: 6 }}>SUBMIT GAME SCORE</div>
          <div style={{ fontSize: 13, color: "#94A3B8", marginBottom: 4 }}>Court 4 · SF-2 · Patel/Lee vs Martin/Clark</div>
          <div style={{ fontSize: 11, color: "#64748B", marginBottom: 10 }}>No referee — submit final game scores</div>
          <button style={{ width: "100%", padding: 10, background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 8, color: "#F59E0B", fontFamily: "Outfit", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>Enter Game {matches.find(m => m.id === 4)?.gameNum || 1} Score →</button>
        </div>
        <div style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 2, marginBottom: 8 }}>LIVE ACROSS TOURNAMENT</div>
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8 }}>
          {active.map(m => (
            <button key={m.id} onClick={() => onViewMatch(m.id)} style={{ minWidth: 150, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "10px 12px", cursor: "pointer", color: "#E8ECF4", textAlign: "left", flexShrink: 0 }}>
              <div style={{ fontFamily: "JetBrains Mono", fontSize: 9, color: m.scoredBy === "referee" ? "#22C55E" : "#64748B", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>{m.scoredBy === "referee" && <LiveDot />}Ct {m.court}</div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#94A3B8" }}><span>{teamDisplay(m.teamA, true)}</span><span style={{ fontFamily: "Outfit", fontWeight: 700, color: "#60A5FA" }}>{m.scoredBy === "referee" ? m.currentGame.a : m.matchScore.a}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#94A3B8", marginTop: 2 }}><span>{teamDisplay(m.teamB, true)}</span><span style={{ fontFamily: "Outfit", fontWeight: 700, color: "#F87171" }}>{m.scoredBy === "referee" ? m.currentGame.b : m.matchScore.b}</span></div>
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "#0A0F1A" }}>
      <div style={{ padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid rgba(234,179,8,0.15)", flexShrink: 0 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#64748B", cursor: "pointer", fontSize: 14 }}>← Back</button>
        <div style={{ textAlign: "center" }}><div style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: COLORS.referee.primary, letterSpacing: 2 }}>COURT 1 · LIVE SCORING</div><div style={{ fontSize: 11, color: "#475569" }}>{match.division} — {match.round}</div></div>
        <div style={{ fontFamily: "JetBrains Mono", fontSize: 12, color: "#64748B" }}>{formatTime(elapsed)}</div>
      </div>
      <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: "8px 0", background: "rgba(234,179,8,0.04)", flexShrink: 0 }}>
        <span style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: "#94A3B8" }}>Game {match.gameNum}/3</span><span style={{ color: "rgba(255,255,255,0.1)" }}>|</span><span style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: "#94A3B8" }}>Match: {match.matchScore.a}-{match.matchScore.b}</span>
        {match.games.length > 0 && <><span style={{ color: "rgba(255,255,255,0.1)" }}>|</span><span style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#475569" }}>{match.games.map((g, i) => `G${i + 1}: ${g.a}-${g.b}`).join("  ")}</span></>}
      </div>
      {isMP && <div style={{ padding: 7, textAlign: "center", fontFamily: "Outfit", fontWeight: 700, fontSize: 12, background: "linear-gradient(90deg, transparent, rgba(234,179,8,0.2), transparent)", color: COLORS.referee.primary, letterSpacing: 2, animation: "pulse 1.5s infinite", flexShrink: 0 }}>⚡ {match.matchScore[match.currentGame.a > match.currentGame.b ? "a" : "b"] === 1 ? "MATCH" : "GAME"} POINT</div>}
      {match.status === "active" ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3, padding: 3, minHeight: 0 }}>
          {["a", "b"].map(t => (
            <button key={t} onClick={() => scorePoint(match.id, t)} style={{
              ...
              <div style={{ fontSize: 13, color: "#94A3B8", marginBottom: 6 }}>{teamDisplay(match[t === "a" ? "teamA" : "teamB"])}</div>
              ...
              <div style={{ position: "absolute", bottom: 14, fontSize: 11, color: "#334155", fontFamily: "JetBrains Mono" }}>TAP TO SCORE</div>
            </button>
          ))}
        </div>
      ) : isGE ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, animation: "slideUp 0.3s ease" }}>
          <div style={{ fontFamily: "JetBrains Mono", fontSize: 13, color: "#22C55E", letterSpacing: 2 }}>GAME {match.gameNum} COMPLETE</div>
          <div style={{ fontFamily: "Outfit", fontSize: 56, fontWeight: 800 }}>{match.currentGame.a} - {match.currentGame.b}</div>
          <button onClick={() => startNextGame(match.id)} style={{ padding: "14px 44px", background: "linear-gradient(135deg, #22C55E, #16A34A)", border: "none", borderRadius: 12, color: "white", fontFamily: "Outfit", fontWeight: 700, fontSize: 17, cursor: "pointer" }}>Start Game {match.gameNum + 1} →</button>
        </div>
      ) : isME ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, animation: "slideUp 0.3s ease" }}>
          <div style={{ fontSize: 15, color: COLORS.referee.primary, fontFamily: "JetBrains Mono", letterSpacing: 3 }}>🏆 MATCH COMPLETE</div>
          <div style={{ fontFamily: "Outfit", fontSize: 22, fontWeight: 600, color: "#94A3B8" }}>{teamDisplay(match.matchScore.a > match.matchScore.b ? match.teamA : match.teamB)} wins</div>
          <div style={{ fontFamily: "Outfit", fontSize: 44, fontWeight: 800 }}>{match.matchScore.a} - {match.matchScore.b}</div>
          <button onClick={onBack} style={{ padding: "14px 44px", background: COLORS.referee.gradient, border: "none", borderRadius: 12, color: "#0A0F1A", fontFamily: "Outfit", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>Submit Result ✓</button>
        </div>
      ) : null}
      {match.status === "active" && <div style={{ padding: "10px 16px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "center", flexShrink: 0 }}><button onClick={undoPoint} style={{ padding: "10px 28px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#94A3B8", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>↩ Undo Last Point</button></div>}
    </div>
  );
}
...
// ORGANIZER: COMP LIST
// ══════════════════════════════════════
function OrgCompList({ onSelect, onBack }) {
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      <TopBar onBack={onBack} title="My Events" />
      <div style={{ padding: 16 }}>
        <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 2, marginBottom: 10 }}>YOUR EVENTS</div>
        {ORG_COMPETITIONS.map((comp, i) => {
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 20, color: COLORS.organizer.primary }}>{comp.playerCount}{isReg ? `/${comp.maxPlayers}` : ""}</div>
                  <div style={{ fontSize: 10, color: "#475569", fontFamily: "JetBrains Mono" }}>{isReg ? "registered" : "players"}</div>
                </div>
              </div>
            </button>
          );
        })}
        <button style={{ width: "100%", padding: 16, background: "rgba(168,85,247,0.06)", border: "1px dashed rgba(168,85,247,0.3)", borderRadius: 14, cursor: "pointer", color: COLORS.organizer.primary, fontFamily: "Outfit", fontWeight: 600, fontSize: 15, marginTop: 4 }}>+ Create New Competition</button>
      </div>
    </div>
  );
}
...
function CategoryList({ comp, onSelect, onBack }) {
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      ...
      <div style={{ padding: 16 }}>
        <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 2, marginBottom: 12 }}>CATEGORIES</div>
        {comp.categories.map((cat, i) => {
          ...
          return (
            ...
            >
              <div><div style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 16 }}>{cat}</div><div style={{ fontSize: 12, color: "#64748B", marginTop: 2 }}>{players.length} players · {metric}</div></div>
              <span style={{ color: "#475569", fontSize: 18 }}>→</span>
            </button>
          );
        })}
...
function OrgDashboard({ matches, category, comp, onBack }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A" }}>
      <TopBar onBack={onBack} title={category} right={isLive ? <span style={{ fontFamily: "JetBrains Mono", fontSize: 10, color: "#22C55E" }}>● LIVE</span> : <Badge text="REG" color={COLORS.organizer.primary} bg={COLORS.organizer.bg} />} borderColor="rgba(168,85,247,0.15)" />
        ...
        <div style={{ display: "flex", gap: 1, padding: "8px 12px 0" }}>
          {[{ l: "Active", v: catMatches.filter(m => m.status === "active").length, c: "#22C55E" }, { l: "Done", v: catMatches.filter(m => m.status === "completed" || m.status === "match_end").length, c: "#3B82F6" }, { l: "Players", v: catPlayers.length, c: COLORS.organizer.primary }, { l: "Checked In", v: catPlayers.filter(p => p.checkedIn).length, c: "#F59E0B" }].map((s, i) => (
            <div key={i} style={{ flex: 1, padding: "8px 10px", background: "rgba(255,255,255,0.03)", borderRadius: i === 0 ? "10px 0 0 10px" : i === 3 ? "0 10px 10px 0" : 0 }}>
              <div style={{ fontFamily: "JetBrains Mono", fontSize: 9, color: "#475569" }}>{s.l}</div>
              <div style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 20, color: s.c }}>{s.v}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: "flex", padding: "10px 12px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            color: tab === t ? "#E8ECF4" : "#475569", fontFamily: "DM Sans", fontWeight: 600, fontSize: 12, cursor: "pointer", textTransform: "capitalize"
          }}>{t}</button>
        ))}
      </div>
              ...
              return (
                <div key={m.id} style={{ background: done ? "rgba(255,255,255,0.02)" : "rgba(168,85,247,0.04)", border: `1px solid ${done ? "rgba(255,255,255,0.05)" : "rgba(168,85,247,0.15)"}`, borderRadius: 12, padding: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>{isA && <LiveDot />}<span style={{ fontFamily: "Outfit", fontWeight: 700, fontSize: 14 }}>Court {m.court}</span></div>
                    <span style={{ fontFamily: "JetBrains Mono", fontSize: 9, color: "#475569" }}>{done ? "FINAL" : m.scoredBy === "referee" ? "🏁 Ref" : "🏓 Player"}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#94A3B8" }}><span>{teamDisplay(m.teamA, true)}</span><span style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 22, color: "#60A5FA" }}>{m.scoredBy === "referee" && isA ? m.currentGame.a : m.matchScore.a}</span></div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#94A3B8", marginTop: 2 }}><span>{teamDisplay(m.teamB, true)}</span><span style={{ fontFamily: "Outfit", fontWeight: 800, fontSize: 22, color: "#F87171" }}>{m.scoredBy === "referee" && isA ? m.currentGame.b : m.matchScore.b}</span></div>
                  <div style={{ marginTop: 6, fontFamily: "JetBrains Mono", fontSize: 9, color: "#334155" }}>{m.round} · Match: {m.matchScore.a}-{m.matchScore.b}</div>
                </div>
              );
            })}
          </div>
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <select value={companyFilter} onChange={e => setCompanyFilter(e.target.value)} style={{
                padding: "6px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(168,85,247,0.2)",
                borderRadius: 8, color: "#94A3B8", fontSize: 12, outline: "none", cursor: "pointer"
              }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
              <button style={{ padding: "7px 14px", background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.2)", borderRadius: 8, color: COLORS.organizer.primary, fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
                <span>👤+</span> Assign Referee
              </button>
              <button onClick={() => alert(`Exported ${filteredPlayers.length} players to CSV`)} style={{ padding: "7px 14px", background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 8, color: "#22C55E", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
                <span>📥</span> Export List ({filteredPlayers.length})
              </button>
              <button onClick={() => { setShowBroadcast(!showBroadcast); setBroadcastSent(false); }} style={{ padding: "7px 14px", background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 8, color: "#3B82F6", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
            ...
            {showBroadcast && (
              <div style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 12, padding: 14, marginBottom: 12, animation: "slideUp 0.2s ease" }}>
                {!broadcastSent ? (<>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#3B82F6", marginBottom: 8 }}>📢 Broadcast to {filteredPlayers.length} {isLive ? (statusFilter === "checked_in" ? "checked-in" : statusFilter === "not_yet" ? "not-checked-in" : "") : (statusFilter === "paid" ? "paid" : statusFilter === "registered" ? "registered" : "")} players{companyFilter !== "all" ? ` at ${companyFilter}` : ""}</div>
                  <textarea value={broadcastMsg} onChange={e => setBroadcastMsg(e.target.value)} placeholder="Type your message..." rows={3} style={{
                    width: "100%", padding: "10px 12px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(59,130,246,0.15)",
                    borderRadius: 8, color: "#E8ECF4", fontSize: 14, outline: "none", resize: "vertical", fontFamily: "DM Sans"
                  }} />
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 10 }}>
                    <button onClick={() => setShowBroadcast(false)} style={{ padding: "7px 16px", background: "none", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#64748B", fontSize: 12, cursor: "pointer" }}>Cancel</button>
                    <button onClick={() => setBroadcastSent(true)} style={{ padding: "7px 16px", background: "linear-gradient(135deg, #3B82F6, #2563EB)", border: "none", borderRadius: 8, color: "white", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Send Message →</button>
                  </div>
                </>) : (
                  <div style={{ textAlign: "center", padding: "8px 0" }}>
                    <div style={{ fontSize: 24, marginBottom: 6 }}>✓</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: "#22C55E" }}>Broadcast sent to {filteredPlayers.length} players</div>
                    <button onClick={() => setShowBroadcast(false)} style={{ marginTop: 8, padding: "6px 16px", background: "none", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#64748B", fontSize: 12, cursor: "pointer" }}>Dismiss</button>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Player count */}
            <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#475569", letterSpacing: 1, marginBottom: 8 }}>
            ...
            {filteredPlayers.map((pl, i) => (
              <div key={i} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#475569", display: "flex", gap: 6, flexWrap: "wrap", marginTop: 2 }}>
                    <span>{pl.email}</span>
                    {pl.partner && <span>· Partner: {pl.partner}</span>}
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  <Badge text={pl.company} color="#64748B" bg="rgba(255,255,255,0.04)" />
                  {isLive ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: pl.checkedIn ? "#22C55E" : "#475569" }} />
                      <span style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: pl.checkedIn ? "#22C55E" : "#64748B", minWidth: 65 }}>{pl.checkedIn ? "Checked In" : "Not Yet"}</span>
                    </div>
                  ) : (
                    <Badge
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {[{ teams: "Garcia/Kim vs. Winner SF-2", round: "Final", eta: "~25 min" }, { teams: "TBD vs. TBD", round: "3rd Place", eta: "~30 min" }].map((m, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10 }}>
                <div style={{ width: 24, height: 24, borderRadius: 6, background: "rgba(245,158,11,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Outfit", fontWeight: 700, fontSize: 12, color: "#F59E0B" }}>{i + 1}</div>
                <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{m.teams}</div><div style={{ fontSize: 10, color: "#475569", fontFamily: "JetBrains Mono" }}>{m.round}</div></div>
                <span style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: "#F59E0B" }}>{m.eta}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {(isLive ? [
              ...
              <div key={i} style={{
                ...
                borderLeft: `3px solid ${a.type === "warning" ? "#EF4444" : a.type === "success" ? "#22C55E" : "#334155"}`
              }}>
                <div style={{ color: "#E8ECF4" }}>{a.text}</div>
                <div style={{ fontFamily: "JetBrains Mono", fontSize: 9, color: "#475569", marginTop: 3 }}>{a.time}</div>
              </div>
            ))}
          </div>
        )}
